function __nx_modern_run
    if test -e nx.json; and type -q jq
        ...
        echo $allTargets | jq -s 'add | .[]'
    end
end

function __nx_run
    if test -e workspace.json; and type -q jq
        jq -r '.projects | to_entries | map("\(.key as $project | .value.targets | select( . != null ) | keys | map("\($project):\(.)") | .[])") | .[]' workspace.json
    end
end
...
complete -f -c nx -n "__fish_seen_subcommand_from run; and not __fish_seen_subcommand_from (__nx_modern_run)" -a "(__nx_modern_run)"
complete -f -c nx -n "__fish_seen_subcommand_from (__nx_modern_run); and not __fish_seen_subcommand_from --" -a "--"